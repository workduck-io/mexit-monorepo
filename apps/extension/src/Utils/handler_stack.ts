...
class HandlerStack {
  ...
  continueBubbling: boolean
  debug: boolean
  eventNumber: number
  stack: any[]
  counter: number
  passEventToPage: Object
  suppressPropagation: Object
  restartBubbling: Object
  currentId: any
  constructor() {
    this.debug = false
    this.eventNumber = 0
      ...
      if (!(handler != null ? handler.id : undefined) || !handler[type]) {
        ...
        if (result === this.passEventToPage) {
          ...
          if (this.isChromeEvent(event)) {
            if (type === 'keydown') {
              DomUtils.consumeKeyup(event, null, false)
            } else {
...
export { HandlerStack }
export let handlerStack = new HandlerStack()
