...
export function getAllVisibleTags(requireHref: boolean) {
  ...
  while ((visibleElement = visibleElements.pop())) {
    ...
    const elementFromMiddlePoint = getElementFromPoint(rect.left + (rect.width * 0.5),
      rect.top + (rect.height * 0.5), null, null);
...
function getElementFromPoint(x, y, root, stack) {
  ...
  if (element && element.shadowRoot)
    return getElementFromPoint(x, y, element.shadowRoot, stack);
