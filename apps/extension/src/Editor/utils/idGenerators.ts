...
export const generateNodeUID = () =>
  `${NODE_ID_PREFIX}${ID_SEPARATOR}${nanoid()}`;
export const generateNodeId = () =>
  `${NODE_ID_PREFIX}${ID_SEPARATOR}${nanoid()}`;
export const generateWorkspaceId = () =>
  `${WORKSPACE_ID_PREFIX}${ID_SEPARATOR}${nanoid()}`;
export const generateIgId = () => `${IG_ID_PREFIX}${ID_SEPARATOR}${nanoid()}`;
export const generateSyncBlockId = () =>
  `${SYNC_BLOCK_ID_PREFIX}${ID_SEPARATOR}${nanoid()}`;
export const generateTempId = () =>
  `${TEMP_ID_PREFIX}${ID_SEPARATOR}${nanoid()}`;
export const generateSnippetId = () =>
  `${SNIPPET_PREFIX}${ID_SEPARATOR}${nanoid()}`;
export const generateSyncTempId = () =>
  `${SYNCTEMP_PREFIX}${ID_SEPARATOR}${nanoid()}`;
export const generateTodoId = () => `${TODO_PREFIX}${ID_SEPARATOR}${nanoid()}`;
export const generateQuestionId = () =>
  `${QUESTION_ID_PREFIX}${ID_SEPARATOR}${nanoid()}`;

export const cleanString = (str: string) =>
  str.startsWith(`${DRAFT_PREFIX}${SEPARATOR}`)
    ? str.replace(`${DRAFT_PREFIX}${SEPARATOR}`, '')
    : str;
