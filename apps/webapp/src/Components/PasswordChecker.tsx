...
const PasswordChecker = ({ password, show }) => {
  ...
  useEffect(() => {
    checkRequirements()
    if (lengthRegex.test(password) && upperRegex.test(password)) {
      setStatus('Weak')
      setCount(1)
      setColor('#FF1744')
    }
    if (lengthRegex.test(password) && upperRegex.test(password) && lowerRegex.test(password)) {
      setStatus('Medium')
      setCount(2)
      setColor('#FF1744')
    }
